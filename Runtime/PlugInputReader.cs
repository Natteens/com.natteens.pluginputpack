...
namespace PlugInputPack
{
    ...
    public class PlugInputReader : ScriptableObject
    {
        ...
        public string GetDebugInfo()
        {
            ...
            int total = 0;
            foreach (var map in inputActionAsset.actionMaps) total += map.actions.Count;
            return $"Maps: {inputActionAsset.actionMaps.Count}, Actions: {total}";
        }

#if UNITY_EDITOR
        private void OnValidate() => deviceSwitchCooldown = Mathf.Clamp(deviceSwitchCooldown, 0f, 2f);
#endif
    }
}
